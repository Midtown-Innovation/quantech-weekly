...
这题主要还是理解动态规划（Dynamic Programming），金融工程中动态规划可以用于奇异期权定价中的有限差分方法，二项式树法等场景。动态规划的实现主要有两种（[via](https://www.javatpoint.com/dynamic-programming "Dynamic Programming")），
